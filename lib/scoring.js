...
function scoreStock(data, nifty60dHistory) {
  ...
  // STRICT FRESH 100/200-DMA cross: ONLY the latest daily candle closed above the MA
  let barsAbove100 = 0, barsAbove200 = 0;
  const l100 = last(sma100);
  if (l100) { for (let ci = n - 1; ci >= 0; ci--) { if (sma100[ci] != null && closes[ci] > sma100[ci]) barsAbove100++; else break; } }
  if (l200) { for (let ci = n - 1; ci >= 0; ci--) { if (sma200[ci] != null && closes[ci] > sma200[ci]) barsAbove200++; else break; } }
  const crossed100 = (barsAbove100 === 1);
  const crossed200 = (barsAbove200 === 1);
  if (crossed100) techScore += 8;   // fresh cross bonus (early entry)
  if (crossed200) techScore += 10;
  ...
  let momReason = '';
  // === ANTI-SPIKE GUARD (1-week swing focus): reward EARLY entries, penalize chasing ===
  let runup5d = 0, spikeLevel = 0;
  const chgNow = n >= 2 ? ((closes[n - 1] - closes[n - 2]) / closes[n - 2] * 100) : 0;
  if (n >= 5) {
    runup5d = (closes[n - 1] - closes[n - 5]) / closes[n - 5] * 100;
    if (runup5d > 8) { momScore -= 18; momReason += `Already ran +${fmt(runup5d, 1)}% in 1 week — spike-chase risk, late entry. `; }
    else if (runup5d > 4) { momScore -= 10; momReason += `Extended +${fmt(runup5d, 1)}% in 1 week — most of the move may be done. `; }
    else if (runup5d > 1) { momScore += 8; momReason += `Early momentum +${fmt(runup5d, 1)}% — move just starting, ideal entry zone. `; }
    else if (runup5d < -5) { momScore -= 12; momReason += `Weak 1-week ${fmt(runup5d, 1)}%. `; }
    else if (runup5d < -2) { momScore -= 5; momReason += `Negative 1-week ${fmt(runup5d, 1)}%. `; }
    else { momScore += 3; momReason += `Flat 1-week (${fmt(runup5d, 1)}%) — coiled, no spike yet. `; }
  }
  if (chgNow > 4) { momScore -= 8; momReason += `Today already +${fmt(chgNow, 1)}% — avoid chasing today's spike. `; }
  spikeLevel = (runup5d > 8 || chgNow > 5) ? 2 : (runup5d > 4 || chgNow > 3.5) ? 1 : 0;
  if (n >= 21) {
    const r1m = (closes[n - 1] - closes[n - 21]) / closes[n - 21] * 100;
    if (r1m > 10) { momScore += 4; momReason += `Strong 1-month +${fmt(r1m, 1)}%. `; }
    else if (r1m > 4) { momScore += 2; momReason += `Positive 1-month +${fmt(r1m, 1)}%. `; }
...
// ============================================================
function getSignal(s, beta, regime, spikeLevel) {
  let base;
  ...
  // Anti-spike cap: no Strong Buy after the move already happened
  if (spikeLevel >= 2 && (base.key === 'strong-buy' || base.key === 'buy')) base = { label: 'Hold', key: 'hold', emoji: '⏳', downgraded: true, spiked: true };
  else if (spikeLevel === 1 && base.key === 'strong-buy') base = { label: 'Buy', key: 'buy', emoji: '✅', downgraded: true, spiked: true };
