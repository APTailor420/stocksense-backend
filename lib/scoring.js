...
function scoreStock(data, nifty60dHistory) {
  ...
  // ============================================================
  // FINAL SCORE — v3/v4 style additive + freshness bonus
  // ============================================================
  techScore = Math.max(0, Math.min(100, Math.round(techScore)));
  momScore = Math.max(0, Math.min(100, Math.round(momScore)));
  let totalScore = Math.round(techScore * 0.55 + momScore * 0.30 + sustainScore * 0.15);

  // Small freshness bonus (not a requirement, just a bonus)
  if (freshGatesPassed >= 4) totalScore = Math.min(100, totalScore + 8);
  else if (freshGatesPassed >= 3) totalScore = Math.min(100, totalScore + 5);
  else if (freshGatesPassed >= 2) totalScore = Math.min(100, totalScore + 3);
  // NO staleness penalty — stocks already in uptrend should still score well
  ...
  // Build reasons — show key bullish indicators (v3/v4 style)
  const reasons = [];
  if (stDir === 1) reasons.push('SuperTrend Bullish');
  if (lm > ls) reasons.push('MACD Bullish');
  if (price > (lVwap || 0)) reasons.push('Above VWAP');
  if (lr < 50) reasons.push('RSI ' + fmt(lr, 0) + ' (room to run)');
  if (l50 && price > l50) reasons.push('Above SMA50');
  if (trendBreak.breakout === 'bullish') reasons.push('Trendline Breakout');
  if (freshGatesPassed >= 1) reasons.push('🔥 ' + freshGatesPassed + ' Fresh Crossover' + (freshGatesPassed > 1 ? 's' : ''));
...
// ============================================================
// SIGNAL CLASSIFICATION — Simple v3/v4 style score-based
// ============================================================
function getSignal(s, beta, regime, gatesPassed, freshGatesPassed) {
  let base;

  // Simple score-based thresholds (v3/v4 style — what was working well)
