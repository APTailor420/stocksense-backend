...
async function saveScan(scanData) {
  ...
  const doc = {
    ...
    // Store FULL result objects so cached scans keep every field the UI needs
...
/**
 * Get the most recent scan if newer than maxAgeMs (cache for /api/scan)
 */
async function getRecentScan(maxAgeMs = 15 * 60 * 1000) {
  const db = await connectDB();
...
module.exports = {
  ...
  saveScan, getRecentScan, getScanHistory, getScanById,
