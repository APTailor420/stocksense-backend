...
const { saveScan, getRecentScan } = require('../lib/db');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const forceFresh = body?.fresh === true || (req.query && req.query.fresh === '1');

    // CACHE: reuse the last scan if recent. Without this every click ran a full
    // 138-stock live scan, so overlapping requests timed out and returned nothing.
    const CACHE_MS = 15 * 60 * 1000;
    if (!forceFresh) {
      try {
        const cached = await getRecentScan(CACHE_MS);
        if (cached && Array.isArray(cached.results) && cached.results.length) {
          return res.status(200).json({
            success: true,
            cached: true,
            cacheAgeSec: Math.round((Date.now() - new Date(cached.timestamp).getTime()) / 1000),
            scanId: cached._id,
            timestamp: cached.timestamp,
            regime: cached.regimeData || { regime: cached.regime },
            portfolioMode: cached.portfolioMode || portfolioMode,
            totalScanned: cached.results.length,
            totalResults: cached.results.length,
            totalErrors: 0,
            results: cached.results
          });
        }
      } catch (e) { console.error('Cache lookup failed:', e.message); }
    }
